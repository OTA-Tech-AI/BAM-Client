...
from browser_use.agent.next_goal_saver import GLOBAL_NEXT_GOAL_SAVER_LIST, \
												reset_global_next_goal_saver
from browser_use.agent.page_content_cleaning import cleanup_task_history

load_dotenv()
logger = logging.getLogger(__name__)


def log_response(response: AgentOutput, concurrent_id: str) -> None:
	...
	GLOBAL_NEXT_GOAL_SAVER_LIST[concurrent_id].set_next_goal(response.current_state.next_goal)
...
class Agent(Generic[Context]):
	...
	def __init__(
		...
		if controller is None:
			self.controller = Controller(concurrent_agent_id=self.concurrent_agent_id)
		else:
	...
	async def step(self, step_info: Optional[AgentStepInfo] = None) -> None:
		...
		try:
			...
			self._message_manager.add_state_message(state, self.state.last_result, step_info, self.settings.use_vision, self.concurrent_agent_id)
			...
			input_messages = cleanup_task_history(input_messages, self.concurrent_agent_id)
	...
	async def get_next_action(self, input_messages: list[BaseMessage]) -> AgentOutput:
		...
		if self.tool_calling_method == 'raw':
			...
			# response: dict[str, Any] = await structured_llm.ainvoke(input_messages, config={"callbacks": [DebugCallbackHandler()]})  # type: ignore
		...
		log_response(parsed, self.concurrent_agent_id)
	...
	async def run(self, max_steps: int = 100) -> AgentHistoryList:
		...
		try:
			...
			reset_global_next_goal_saver(self.concurrent_agent_id)
			self.telemetry.capture(
				AgentEndTelemetryEvent(
					agent_id=self.state.agent_id,
	...
	async def _validate_output(self) -> bool:
		...
		if self.browser_context.session:
			...
			GLOBAL_NEXT_GOAL_SAVER_LIST[self.concurrent_agent_id].set_next_goal(self.task)
			msg = [SystemMessage(content=system_msg), content.get_user_message(self.settings.use_vision, self.concurrent_agent_id)]
		else:
